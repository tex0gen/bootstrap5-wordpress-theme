...
function update_plugin_json($option_name, $old_value, $value) {
	if (strpos($currentURL, 'test.') !== true || strpos($currentURL, 'dev.') !== true) {
	  ...
	  if ( 'active_plugins' == $option_name ) {
	    $fp = fopen(get_stylesheet_directory() . '/inc/plugins/plugin_list.json', 'w');
	    if ($value) {
	      foreach ($value as $key => $plugin) {
	        $plugin_info = get_plugin_data( WP_PLUGIN_DIR . '/' . $plugin);

	        if (in_array($plugin_info['TextDomain'], $premium_plugins)) {
	          $plgns[$key]['name'] = $plugin_info['Name'];
	          $plgns[$key]['slug'] = $plugin_info['TextDomain'];
	          $plgns[$key]['source'] = 'http://plugins.wonderfulserver.com/' . $plugin_info['TextDomain'] . '.zip';
	          $plgns[$key]['required'] = true;
	        } else {
	          $plgns[$key]['name'] = $plugin_info['Name'];
	          $plgns[$key]['slug'] = $plugin_info['TextDomain'];
	          $plgns[$key]['required'] = true;
...
function my_theme_register_required_plugins() {
	 ...
	 */
	$plugin_list = file_get_contents(__DIR__ . '/plugin_list.json');
	$plugin_list_decoded = json_decode($plugin_list, true);
	$plugins = $plugin_list_decoded;
